...
const Maps = () => {
	...
	useEffect(() => {
		...
		fetch("https://gw.alipayobjects.com/os/antvdemo/assets/data/world.geo.json")
			...
			.then(mapData => {
				fetch(
					"https://gw.alipayobjects.com/os/antvdemo/assets/data/earthquake.json"
				)
					.then(res => res.json())
					.then(data => {
						...
						chart.tooltip({
							...
							itemTpl:
								'<tr data-index="{index}"><td style="padding:5px;background-color:#fff;">{name}</td><td style="padding:5px;background-color:#fff;color:#000;">{value}</td></tr>',
							domStyles: {
								"g2-tooltip": {
									borderRadius: "2px",
